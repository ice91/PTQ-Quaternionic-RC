...
import numpy as np
from .constants import C_LIGHT, H0_SI, KPC, KM, G_SI, MSUN
...
def model_v_kms_mond(Upsilon: float,
                     a0_si: float,
                     r_kpc: np.ndarray,
                     v_disk_kms: np.ndarray,
                     v_bulge_kms: np.ndarray,
...
def nfw_v_kms_M200_c(r_kpc: np.ndarray,
    ...
    M_r = M200 * (g / gc)
    v2  = G_SI * M_r / (r_kpc * KPC)          # [m^2 s^-2]
    return np.sqrt(v2) / KM                   # [km/s]

def c_powerlaw(M200_Msun: float,
               c0: float = 10.0,
               beta: float = -0.1,
               Mpiv: float = 1.0e12) -> float:
    return c0 * (M200_Msun / Mpiv)**beta

def model_v_kms_nfw1p(Upsilon: float,
                      log10_M200: float,
                      r_kpc: np.ndarray,
                      v_disk_kms: np.ndarray,
                      v_bulge_kms: np.ndarray,
    ...
    """
    v_model = sqrt( v_bar^2 + v_nfw^2(M200, c(M200)) )
    M200 is in Msun (passed as log10).
    """
    M200 = 10.0**log10_M200
    c    = c_powerlaw(M200, c0=c0, beta=c_slope)
    vbar2 = Upsilon * (v_disk_kms**2 + v_bulge_kms**2) + v_gas_kms**2
