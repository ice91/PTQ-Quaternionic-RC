...
def _guess_scale_1e9(series: pd.Series) -> float:
    """若數值多半 < 1e6，視為以 10^9 單位提供，回傳 1e9；否則回傳 1."""
...
def make_btfr(results_dir: Path, data_csv: Path, out_name: str = "btfr.png") -> Path:
    ...
    # 準備單位偵測（支援 L36_tot / L36_disk / L36_bulge / M_HI）
    s_Ltot = _guess_scale_1e9(base_grp["L36_tot"]) if "L36_tot" in base_grp.columns else 1.0
    s_Ld   = _guess_scale_1e9(base_grp["L36_disk"]) if "L36_disk" in base_grp.columns else 1.0
    s_Lb   = _guess_scale_1e9(base_grp["L36_bulge"]) if "L36_bulge" in base_grp.columns else 1.0
    ...
    for name, g in gdict.items():
        ...
        Ltot = row.get("L36_tot",   np.nan)
        MHI  = row.get("M_HI",      row.get("MHI", row.get("Mgas", np.nan)))

        # 單位縮放（若是 10^9 單位，統一換成 Lsun/Msun）
        if np.isfinite(Ld):   Ld   = float(Ld)   * s_Ld
        if np.isfinite(Lb):   Lb   = float(Lb)   * s_Lb
        if np.isfinite(Ltot): Ltot = float(Ltot) * s_Ltot
        if np.isfinite(MHI):  MHI  = float(MHI)  * s_HI

        # 決定 M_star：優先用 disk/bulge 分量；否則用總亮度
        Mstar = np.nan
        if np.isfinite(U) and np.isfinite(Ld):
            # 若同時有 bulge 欄，且有 Ub，就加總；否則只用 disk
            part_bulge = (Ub * Lb) if (np.isfinite(Ub) and np.isfinite(Lb)) else 0.0
            Mstar = U * Ld + part_bulge
        elif np.isfinite(U) and np.isfinite(Ltot):
            # 只有總亮度
            Mstar = U * Ltot
    ...
    if not rows:
        raise RuntimeError("BTFR 需要 L36_tot（或 L36_disk/L36_bulge）以及 M_HI；目前在 CSV 中找不到可用欄位。")
